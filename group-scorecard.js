...
import { openScorecardExportPicker } from './scorecard-export.js?v=2';
...
async function handleClick(event){
  ...
  if(event.target.closest('[data-export-scorecard]')){if(round.status!=='completed')return;try{const when=new Date(round.scheduled_at);openScorecardExportPicker({course:round.course_name,date:round.scheduled_at,tee:round.tee_name,teeTime:when.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),holeCount:holeCount(),par:round.par,participants:round.participants.map(player=>({id:player.id,name:personLabel(player),holes:scores(player)}))})}catch(error){setMessage(error.message||'That scorecard image could not be created.',true)}return}
